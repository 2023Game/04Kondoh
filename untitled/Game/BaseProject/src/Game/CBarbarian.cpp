...
#define KICK_KNOCKBACK		50.0f	// 回し蹴りのノックバック距離
#define TACKLE_KNOCKBACK	50.0f	// タックルのノックバック距離
#define HEADBUTT_KNOCKBACK	50.0f	// 頭突きのノックバック距離
...
#define GUARD_PROB	25		// 防御の確率
#define AVOID_PROB	50		// 回避の確率
...
#define AVOID_MOVE_END		38.0f	// 回避時の移動終了フレーム
...
const std::vector<CEnemyBase::AnimData> ANIM_DATA =
{
	...
	{ ATTACK_ANIM_PATH"LeftBlow.x",			false,	 81.0f,	0.8f},	// 左薙ぎ払い
	{ ATTACK_ANIM_PATH"RightBlow.x",		false,	 81.0f,	0.8f},	// 右薙ぎ払い
	...
	{ ANIM_PATH"LeftAvoid.x",				false,	 44.0f, 1.0f},	// 左回避
	{ ANIM_PATH"RightAvoid.x",				false,	 44.0f, 1.0f},	// 右回避
...
void CBarbarian::Update()
{
	...
	if (state != nullptr)
	{
		mpDebugFov->SetColor(GetStateColor(state->GetIndex()));
	}

	CDebugPrint::Print("■敵の情報\n");
	CDebugPrint::Print("　HP：%d\n", mHp);
	CDebugPrint::Print("　怯み度：%.2f\n", mStunPoints);
	CDebugPrint::Print("　攻撃フレーム：%f\n", mAnimationFrame);
	if (state != nullptr)
	{
...
void CBarbarian::Render()
{
	...
	{
		// 巡回中であれば
		if (state->GetIndex() == (int)EState::ePatrol)
		{
			float rad = 1.0f;
			// 巡回ポイントを全て描画
			...
			for (int i = 0; i < size; i++)
			{
				...
				CColor c = CColor::cyan;
				Primitive::DrawWireSphere(m, rad, c);
			}
		}
		// 見失ったとき
		else if (state->GetIndex() == (int)EState::eLost)
		{
			//プレイヤーを見失った位置にデバッグ表示
			float rad = 2.0f;
			CMatrix m;
